...
pub struct Task {
    ...
    /// Tracks the lowest stack pointer value (fullest stack) observed on any
    /// kernel entry for this instance of this task.
    /// Initialized to `u32::MAX` if the task has not yet run.
    #[cfg(feature = "stack-watermark")]
    stack_pointer_low: u32,

    /// Tracks the lowest stack pointer value observed across *any* instance
    /// of this task (survives restarts).
    /// Initialized to `u32::MAX` if the task has not yet run.
    #[cfg(feature = "stack-watermark")]
...
impl Task {
    ...
    ///
    /// If not enabled, this is a no-op and should be safe to call freely
